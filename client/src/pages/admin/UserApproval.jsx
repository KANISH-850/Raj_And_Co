...
const UserApproval = () => {
    ...
    const handleAction = async (id, payload) => {
        ...
        } catch { toast.error('Failed to save changes.', { id: tid }); }
    };
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-secondary-900 tracking-tighter flex items-center gap-3">
                        <ShieldCheck className="text-primary-600" size={36} /> Account Approval
                    </h1>
                    <p className="text-secondary-500 font-medium max-w-lg">Manage which workers and staff can access the system.</p>
                </div>
                <div className="flex items-center gap-4">
                     <div className="px-6 py-4 bg-secondary-50 border border-secondary-200 rounded-2xl flex items-center gap-3">
                        ...
                        <span className="text-secondary-900 font-black text-xs uppercase tracking-widest">{users.filter(u => !u.isApproved).length} People Waiting</span>
                     </div>
                </div>
            </header>
                    ...
                    {loading ? (
                       <div className="col-span-full py-20 flex justify-center"><Loader2 className="animate-spin text-primary-500" size={48} /></div>
                        ...
                        <motion.div 
                            ...
                            `}
                        >
                            {!user.isApproved && <div className="absolute top-0 right-0 px-8 py-2 bg-amber-500 text-white text-[10px] font-black uppercase tracking-[0.3em] rounded-bl-[2rem] shadow-lg">New Account</div>}
                            ...
                            <div className="flex gap-6 items-start mb-8">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex flex-col gap-1 mt-1">
                                        ...
                                        <div className="flex items-center gap-2 text-secondary-400 text-[10px] font-black uppercase tracking-tighter"><Calendar size={14} /> Since {new Date(user.createdAt).toLocaleDateString()}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                               <button 
                                 ...
                                 `}
                               >
                                 {user.isApproved ? <><UserX size={16} /> Block Access</> : <><UserCheck size={16} /> Approve Access</>}
                               </button>
                               ...
                               <button 
                                 ...
                                 `}
                               >
                                 {user.role === 'admin' ? <><UserX size={16} /> Remove Admin</> : <><UserPlus size={16} /> Make Admin</>}
                               </button>
                            </div>
                        </motion.div>
                    ))}
